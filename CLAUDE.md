...
**Update 2026-07-19 — host decided: DigitalOcean App Platform** (Render service abandoned). The app spec is `.do/app.yaml` (deploys `ship/integration`; Dockerfile build, migrations run in `render-start.sh` at container start, domain api.agrobridgefoundation.org). `DATABASE_URL` must use `sslmode=no-verify` (this app is on `@prisma/adapter-pg`/node-postgres, which verifies the CA on `require`, and DO's CA is private). Boot hard-requires the three STRIPE_* vars — use test-mode keys until stable. Shared infra bootstrap: `~/Documents/faro-backend/docs/DEPLOY_DIGITALOCEAN.md` §1. Follow-up: the foundation web still points at the dead onrender.com URL.
