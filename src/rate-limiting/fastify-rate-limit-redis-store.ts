...
export function createRedisBackedRateLimitStore(
  ...
  // interface, and the store contract is covered by unit tests.
  return class RedisBackedRateLimitStore {
    private readonly redis: RedisClientType = redis;
    ...
    incr(
      ...
      this.redis
        .sendCommand(['EVAL', INCR_SCRIPT, '1', fullKey, String(timeWindow)])
        .then((res: unknown) => {
          const arr = Array.isArray(res) ? (res as Array<number | string>) : [res as number, timeWindow];
