...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post('/auth/refresh', async (req, reply) => {
    // Verify current token and issue a new one. Accept tokens signed with the
    // previous secret during the rotation window (verifyAdminToken handles
    // the fallback), then re-issue with the CURRENT secret so the client
    // migrates off the old token.
    const currentPayload = await verifyAdminToken(req);
    if (!currentPayload) {
      reply.status(401);
      return fail({ code: 'UNAUTHORIZED', message: 'Invalid or expired token' });
    }

    const payload = currentPayload as {
      sub: string;
      email: string;
      role: 'ADMIN' | 'SUPER_ADMIN';
    };

    // SECURITY: Verify user is not deleted or locked before refreshing.
